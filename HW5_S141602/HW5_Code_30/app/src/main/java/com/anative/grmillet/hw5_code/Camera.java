...
public class Camera {
    ...
    public void Yaw(float dYaw) { // rotate v , n->u
        ...
        uAxis[0] = -nAxis[1] * vAxis[2] + nAxis[2] * vAxis[1];
        uAxis[1] = nAxis[0] * vAxis[2] - nAxis[2] * vAxis[0];
        uAxis[2] = -nAxis[0] * vAxis[1] + nAxis[1] * vAxis[0];
    ...
    public void Roll(float dRoll) { // rotate n , u->v
        ...
        uAxis[0] = cosa * uAxis[0] + sina
                * (-uAxis[1] * nAxis[2] + uAxis[2] * nAxis[1]);
        uAxis[1] = cosa * uAxis[1] + sina
                * (uAxis[0] * nAxis[2] - uAxis[2] * nAxis[0]);
        uAxis[2] = cosa * uAxis[2] + sina
                * (-uAxis[0] * nAxis[1] + uAxis[1] * nAxis[0]);

        vAxis[0] = -uAxis[1] * nAxis[2] + uAxis[2] * nAxis[1];
        vAxis[1] = uAxis[0] * nAxis[2] - uAxis[2] * nAxis[0];
        vAxis[2] = -uAxis[0] * nAxis[1] + uAxis[1] * nAxis[0];
    ...
    public void Zoom(float delta) {
        ...
        if (delta < 0 && fovy > 1.0f)
            fovy += delta * 0.08f;

        if (delta > 0 && fovy < 98.0f)
            fovy += delta * 0.08f;
    }
